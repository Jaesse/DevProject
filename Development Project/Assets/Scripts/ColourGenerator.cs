    ...
    public void UpdateColour()
    {
        ...
        foreach (var biome in settings.biomeColourSettings.biomes)
        {
            for (int i = 0; i < textureResolution * 2; i++)
            {
                ...
                {
                    gradientCol = settings.biomeColourSettings.oceanColour.Evaluate(i / (textureResolution - 1f));
                }
                else
                { 
                ...
                colours[colourIndex] = gradientCol * (1 - biome.tintPerecent) + tintCol * biome.tintPerecent;
                colourIndex++;
